...
Optional-model tests verify the Image Safety XS manifest and strict NSFW filter threshold, preservation of all three scores, unchanged-file reuse, Images/Videos scope selection, bounded one/two-frame video timestamp selection (including the one-/three-second unknown-duration fallback), video priority routing only to Image Safety, obsolete-package cleanup only after verification, and retrying the checked-in repository mirror after an upstream download failure. Credential-config tests verify that generic Python settings containing only complete environment references, booleans and rate-limit names are not reported as exposed credentials, while snake/camel/Pascal-case hard-coded assignments, concrete `getenv`/`setdefault` fallbacks, dollar-prefixed literal secrets and known-prefix tokens under neutral variable names remain detectable.
...
Triage Router coverage verifies scanner applicability, soft down-ranking of thumbnails without exclusion, five-percent selection, the absolute cap, preservation of every applicable route for selected objects, cooperative candidate-preparation cancellation and progress, registry-driven future-scanner execution and local session round-tripping. Manual large-snapshot checks verify that priority, people, NSFW, document, EXIF, PII, secrets, config, identity and archive candidate preparation performs no per-path existence sweep; missing/locked files must become isolated per-object log entries while later objects continue. People/NSFW cancellation must apply only the in-memory completed-result journal and return without a per-candidate SQLite restoration pass. Manual wizard checks cover preparation off the dispatcher, the shared heavy-operation lock, Cancel preparation → Start preparation → Check priority objects transitions, pause/resume/cancel persistence, unavailable-scanner isolation, stage snapshots, read-only controls during execution, dynamic report visibility and return navigation from the priority grid.
