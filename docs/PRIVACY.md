...
Image Safety Classifier XS скачивается только после явного согласия пользователя, проверяется по закреплённому SHA-256 и может быть полностью удалена. NSFW-анализ фото и видео выполняется локально и работает офлайн после установки. Для видео декодируются не более двух кадров; исходные файлы, пути, результаты, кадры, миниатюры и уменьшенные RGB 224×224 данные не отправляются в сеть и не записываются как производные файлы. Кадры и уменьшенные данные существуют только в памяти на время inference.
...
Image Safety Classifier XS is downloaded only after explicit user consent, verified against a pinned SHA-256, and can be removed completely. Photo and video NSFW analysis is local and works offline after installation. At most two video frames are decoded; source files, paths, results, frames, thumbnails, and resized RGB 224×224 inputs are never uploaded or written as derived files. Frames and resized inputs exist only in memory during inference.
