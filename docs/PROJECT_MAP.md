...
- **Collapsible Media deep-analysis panel**: the people, document and metadata scanner cards are expanded on every application start and can be collapsed for the current window session with the subdued `⌜⌟` affordance aligned to the right edge and optically centered vertically. The collapsed surface retains only the affordance-height reopening target; Media filters remain independent and visible.
