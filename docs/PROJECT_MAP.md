...
- Archive sensitivity is an on-demand inspection result, not a pre-scan category filter. The Findings toolbar keeps **«Inspect archives»** as the entry point; after completion the view returns to all findings and shows archive severity badges for the inspected files.
