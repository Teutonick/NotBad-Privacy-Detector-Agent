...
- **Image Safety media controls**: the optional XS package classifies every found image as NSFW, NSFL or SFW and persists all three scores. Its Search/Continue, Pause, Cancel and Remove model controls follow the people-search layout and preserve completed partial results. The sole NSFW Media filter includes only completed results with `NSFW score > 0.85`; a compact background-free eye/eye-off toggle at the far right controls NSFW thumbnail blur. Details exposes the existing confirmed Recycle Bin workflow for files and hides it for directories.
