...
- **Image Safety media controls**: the optional XS package classifies the user-selected Images and/or Videos scope as NSFW, NSFL or SFW and persists all three scores; Images is selected by default and Search is disabled when neither scope is selected. Its Search/Continue, Pause, Cancel and Remove model controls follow the people-search layout and preserve the original candidate set for a paused run. Media provides All images, All videos and All media views; the sole NSFW filter includes completed photo and video results with `NSFW score > 0.85`. Videos use a lightweight glyph in the shared gallery to avoid an extra thumbnail decode. A compact background-free eye/eye-off toggle at the far right controls NSFW thumbnail blur. Details exposes the existing confirmed Recycle Bin workflow for files and hides it for directories.
