...
The startup workspace follows the **Privacy Radar** lifecycle. `SnapshotStore` persists both enriched findings and the audit context (preset, roots, start/completion time and duration). `MainWindow.TryRestoreSnapshotAsync` automatically restores it after first render; JSON decoding, personal-score preparation and ranking run on worker threads while navigation remains rendered in read-only mode. Restore does not re-probe every remembered media path; stale paths remain visible forensic records and are isolated only if a scanner later attempts them. Restore accepts cancellation: the cancel action releases the restore guard and opens the new-audit form immediately, while the old snapshot and audit rows remain until a replacement audit actually starts. The scan-settings card is then replaced with the current-radar summary. **Start over** reveals the standard preset/scope form; old audit rows and the resumable snapshot are cleared only when the replacement audit actually starts. `Core/PrivacyRadarRanking.cs` supplies evidence-aware ordering from accumulated deep-detector metadata.
...
- **Media analysis controls**: the Media workspace presents all found images and videos by default. Its category dropdown is independent from shared Images/Videos gallery switches, while people and NSFW analysis each own separate Images/Videos execution scopes that default to Images only. Candidate preparation uses the audit snapshot without a second filesystem-existence pass. People, NSFW and document-photo detection remain independent local operations with their own progress, pause/continue and cancel controls; completed results are preserved independently, and cancellation consumes an in-memory completed-result journal instead of rereading SQLite for every candidate. The detailed runtime contract is documented in [ARCHITECTURE.md](ARCHITECTURE.md#media-analysis-lifecycle).
- **Compact media progress**: running and completed people/NSFW analysis keeps the description, numeric progress and counters on one ellipsized line; redundant secondary progress text is collapsed so the deep-analysis panel does not grow unnecessarily.
...
- **Image Safety media controls**: the optional XS package classifies the user-selected Images and/or Videos scope as NSFW, NSFL or SFW and persists all three scores; Images is selected by default and Search is disabled when neither scope is selected. Its Search/Continue, Pause, Cancel and Remove model controls mirror the people-search scope/lifecycle and preserve the original candidate set for a paused run. The semantic NSFW gallery category combines with the shared Images/Videos filter and includes completed photo and video results with `NSFW score > 0.85`. Videos use a lightweight glyph in the shared gallery to avoid an extra thumbnail decode. A compact background-free eye/eye-off toggle at the far right controls NSFW thumbnail blur. Details exposes the existing confirmed Recycle Bin workflow for files and hides it for directories.
...
- **Priority deep-check wizard**: after the primary audit, Overview offers a budgeted local pass over a diverse scope of up to 5%, capped at 50,000 objects. Candidate preparation is cancellable and holds the shared heavy-operation lock, disabling other expensive scanners until it finishes or unwinds after cancellation. Its UI follows Cancel preparation → Start preparation → Check priority objects. The limit selects objects, then every selected object retains all applicable scanner routes; progress counts those operations, so one image can contribute people, document, NSFW and EXIF checks. Paused sessions created under an older selection policy are rebuilt instead of resuming an oversized queue. The dynamic Priority Report appears only after completion and reuses the same findings, Details navigation and file context actions without exposing scanner-launch controls.
...
- **Priority Report pagination and image tiles**: filtered/sorted priority findings use the same list/tile presentation contract as Findings. The initial order is Privacy Risk descending; confirmed-detector order is an explicit alternative and means a positive local scanner signal, not human approval. Selecting the Images category reveals the localized **Tiles** switch, zoom control, thumbnail cards, feedback actions and Details navigation; tile scale dynamically determines page size, while list mode keeps 600 rows. Filter and presentation changes return to page one.
