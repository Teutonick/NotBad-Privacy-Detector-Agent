...
public partial class MainWindow : Window
{
    ...
    Finding[] _sortedPriorityFindings = [];
    int _priorityReportPage;
    ...
    readonly ModelManager _imageSafetyModelManager;
    readonly ImageSafetyRepository _imageSafetyRepository;
    ...
    Finding? _selected;
    enum DetailsReturnSource { FindingsGrid, FindingsTiles, MediaTiles, ApplicationHistory, PriorityReport, PriorityReportTiles }
    ...
    bool _mediaPriorityPartialHintDismissed;
    bool _applicationHistoryPriorityPartialHintDismissed;
    ...
    CancellationTokenSource? _priorityPreparationCts;
    bool _priorityPreparationQueued;
    ...
    IDisposable? TryAcquireHeavyTask(string taskName)
    {
        ...
        return new ActionDisposable(() =>
        {
            ...
            Dispatcher.InvokeAsync(() =>
            {
                lock (_heavyTaskLock)
                {
                    if (_activeHeavyTaskName is not null || _activeMediaTaskCount > 0) return;
                }
    ...
    void SetGlobalScanControlsEnabled(bool enabled)
    {
        ...
        if (PriorityWizardPrepareButton is not null) PriorityWizardPrepareButton.IsEnabled = enabled;
        if (PriorityWizardStartButton is not null) PriorityWizardStartButton.IsEnabled = enabled && _priorityAuditSession?.Status != PriorityAuditStatus.Running;
    }
    ...
    public MainWindow(bool suppressRestorePrompt = false)
    {
        ...
        _imageSafetyModelManager = new(data, ModelManifest.ImageSafetyXs);
        _imageSafetyRepository = new(_databasePath);
        _deepScannerRegistry = DeepAuditScannerRegistry.CreateDefault(_modelManager, _peopleRepository, _imageSafetyModelManager, _imageSafetyRepository);
        ...
        PriorityReportTileList.ItemsSource = _visiblePriorityFindings;
        ...
        Loaded += async (_, _) =>
        {
            var modelAvailabilityTask = Task.WhenAll(RefreshPeopleModelAvailabilityAsync(), RefreshImageSafetyModelAvailabilityAsync());
    ...
    (Finding[] Findings, Finding[] MediaFindings) PrepareRestoredSnapshot(ScanSnapshot snapshot)
    {
        ...
        var media = findings.Where(x => x.Category is "Images" or "Video")
    ...
    {
        if (_restoreCts is not null || _priorityPreparationCts is not null || _priorityAuditCts is not null || _cts is not null) return;
    ...
    void ShowPreviousAuditSummary(ScanSnapshot snapshot)
    {
        ...
        _ = QueuePriorityWizardPreparationAsync();
    }

    async Task QueuePriorityWizardPreparationAsync()
    {
        if (_priorityPreparationQueued || _priorityPreparationCts is not null) return;
        _priorityPreparationQueued = true;
        var generation = _priorityWizardPreparationVersion;
        try
        {
            while (true)
            {
                bool busy;
                lock (_heavyTaskLock) busy = _activeHeavyTaskName is not null || _activeMediaTaskCount > 0;
                if (!busy) break;
                await Task.Delay(50);
                if (!IsLoaded || generation != _priorityWizardPreparationVersion || _findings.Count == 0) return;
            }
            await Dispatcher.InvokeAsync(() => { }, System.Windows.Threading.DispatcherPriority.Background);
            if (IsLoaded && generation == _priorityWizardPreparationVersion) await PreparePriorityWizardAsync();
        }
        finally { _priorityPreparationQueued = false; }
    }
    ...
    {
        if (source.AuditFingerprint != CurrentAuditFingerprint()) return;
        var snapshot = CopyPrioritySession(source);
        await _prioritySessionSaveGate.WaitAsync();
        try
        {
            if (snapshot.AuditFingerprint != CurrentAuditFingerprint()) return;
            await Task.Run(() => _priorityAuditStore.Save(snapshot));
        }
    ...
    {
        if (!IsLoaded || _findings.Count == 0 || _priorityAuditCts is not null || _priorityPreparationCts is not null) return;
        ...
        if (_priorityAuditSession is not null && AuditIdentity.Matches(_priorityAuditSession.AuditFingerprint, fingerprint, _auditContext)
            && (_priorityAuditSession.Status == PriorityAuditStatus.Completed || _priorityAuditSession.SelectionPolicyVersion >= PriorityAuditSession.CurrentSelectionPolicyVersion))
        {
        ...
        if (restored is not null && AuditIdentity.Matches(restored.AuditFingerprint, fingerprint, _auditContext)
            && (restored.Status == PriorityAuditStatus.Completed || restored.SelectionPolicyVersion >= PriorityAuditSession.CurrentSelectionPolicyVersion))
        {
        ...
        var guard = TryAcquireHeavyTask(LocalizationService.Get("PriorityWizardPreparingTitle"));
        if (guard is null)
        {
            ShowPriorityWizardPreparationAvailableState(false);
            return;
        }
        _priorityPreparationCts = new CancellationTokenSource();
        var token = _priorityPreparationCts.Token;
        ShowPriorityWizardPreparingState();
        PriorityWizardReadyText.Text = LocalizationService.Get("PriorityWizardPreparing");
        PriorityWizardPlanText.Text = "";
        try
        {
            var findings = _findings.ToArray();
            var progress = new Progress<TriagePreparationProgress>(value =>
            {
                PriorityWizardPreparationProgress.Value = value.Fraction;
                PriorityWizardPreparationProgressText.Text = string.Format(LocalizationService.Get("PriorityWizardPreparationProgress"), value.Completed, value.Total);
            });
            var selection = await Task.Run(() => new TriageRouter().Select(findings, token: token, progress: progress), token);
            token.ThrowIfCancellationRequested();
            if (!IsLoaded || preparationVersion != _priorityWizardPreparationVersion || fingerprint != CurrentAuditFingerprint()) return;
            ...
            _priorityAuditSession = new PriorityAuditSession
            {
                ...
                SelectionPolicyVersion = PriorityAuditSession.CurrentSelectionPolicyVersion,
        ...
        {
            if (preparationVersion == _priorityWizardPreparationVersion) ShowPriorityWizardPreparationAvailableState(true);
        }
        catch (Exception ex)
        {
            CrashLogger.LogException(ex, "Priority candidate preparation");
            if (preparationVersion == _priorityWizardPreparationVersion) ShowPriorityWizardPreparationAvailableState(false);
        }
        finally
        {
            _priorityPreparationCts?.Dispose();
            _priorityPreparationCts = null;
            guard.Dispose();
        }
    }
    ...
    void ShowPriorityWizardPreparingState()
    {
        ...
        PriorityWizardReadyPanel.Visibility = Visibility.Visible;
        PriorityWizardRunningPanel.Visibility = Visibility.Collapsed;
        PriorityWizardCompletePanel.Visibility = Visibility.Collapsed;
        PriorityWizardStartButton.IsEnabled = false;
        PriorityWizardStartButton.Visibility = Visibility.Collapsed;
        PriorityWizardPrepareButton.Visibility = Visibility.Collapsed;
        PriorityWizardCancelPreparationButton.Visibility = Visibility.Visible;
        PriorityWizardCancelPreparationButton.IsEnabled = true;
        PriorityWizardPreparationProgress.Visibility = Visibility.Visible;
        PriorityWizardPreparationProgress.IsIndeterminate = false;
        PriorityWizardPreparationProgress.Value = 0;
        PriorityWizardPreparationProgressText.Visibility = Visibility.Visible;
        PriorityWizardPreparationProgressText.Text = string.Format(LocalizationService.Get("PriorityWizardPreparationProgress"), 0, _findings.Count);
        PriorityWizardReportButton.IsEnabled = false;
        PriorityWizardReportButton.IsHitTestVisible = false;
        PriorityWizardFullReportButton.IsEnabled = false;
        PriorityWizardFullReportButton.IsHitTestVisible = false;
    ...
    void ShowPriorityWizardPreparationAvailableState(bool canceled)
    {
        PriorityAuditWizardPanel.Visibility = Visibility.Visible;
        PrivacyRadarIntroPanel.Visibility = Visibility.Collapsed;
        PriorityWizardReadyPanel.Visibility = Visibility.Visible;
        PriorityWizardRunningPanel.Visibility = Visibility.Collapsed;
        PriorityWizardCompletePanel.Visibility = Visibility.Collapsed;
        PriorityWizardReadyText.Text = LocalizationService.Get(canceled ? "PriorityWizardPreparationCanceled" : "PriorityWizardPreparationReady");
        PriorityWizardPlanText.Text = "";
        PriorityWizardPrepareButton.Visibility = Visibility.Visible;
        PriorityWizardPrepareButton.IsEnabled = true;
        PriorityWizardCancelPreparationButton.Visibility = Visibility.Collapsed;
        PriorityWizardStartButton.Visibility = Visibility.Collapsed;
        PriorityWizardPreparationProgress.Visibility = Visibility.Collapsed;
        PriorityWizardPreparationProgressText.Visibility = Visibility.Collapsed;
    }
    ...
    void UpdatePriorityWizardPresentation()
    {
        ...
        PriorityWizardCompletePanel.Visibility = session.HasReport ? Visibility.Visible : Visibility.Collapsed;
        PriorityWizardStartButton.IsEnabled = session.Status != PriorityAuditStatus.Running;
        PriorityWizardStartButton.Visibility = Visibility.Visible;
        PriorityWizardPrepareButton.Visibility = Visibility.Collapsed;
        PriorityWizardCancelPreparationButton.Visibility = Visibility.Collapsed;
        PriorityWizardPreparationProgress.Visibility = Visibility.Collapsed;
        PriorityWizardPreparationProgressText.Visibility = Visibility.Collapsed;
        ...
        PriorityWizardPlanText.Text = string.Format(LocalizationService.Get("PriorityWizardPlanV2"), session.TotalRoutes, session.Routes.Select(route => route.ScannerId).Distinct(StringComparer.OrdinalIgnoreCase).Count());
        ...
        var reportReady = session.HasReport;
        TabPriorityReport.Visibility = reportReady ? Visibility.Visible : Visibility.Collapsed;
    ...
    async void PriorityWizardPrepare_Click(object sender, RoutedEventArgs e) => await PreparePriorityWizardAsync();

    void PriorityWizardCancelPreparation_Click(object sender, RoutedEventArgs e)
    {
        if (_priorityPreparationCts is null) return;
        PriorityWizardCancelPreparationButton.IsEnabled = false;
        PriorityWizardReadyText.Text = LocalizationService.Get("PriorityWizardPreparationCanceling");
        RequestCancellation(_priorityPreparationCts, showGlobalProgress: false);
    }
    ...
    {
        if (_priorityAuditSession?.HasReport != true)
        {
            TabPriorityReport.Visibility = Visibility.Collapsed;
            UpdatePriorityWizardPresentation();
            return;
    ...
    {
        if (PriorityReportCategoryFilter is null || _priorityAuditSession?.HasReport != true) return;
    ...
    void RefreshPriorityReport(bool resetPage = false)
    {
        if (!IsInitialized || PriorityReportRiskFilter is null) return;
        if (_priorityAuditSession?.HasReport != true)
        {
            _sortedPriorityFindings = [];
            _visiblePriorityFindings.Clear();
        ...
        var sort = (PriorityReportSortFilter.SelectedItem as ComboBoxItem)?.Tag?.ToString() ?? "Privacy";
        ...
        _sortedPriorityFindings = source.ToArray();
        if (resetPage) _priorityReportPage = 0;
        UpdatePriorityReportPresentation();
        var pageSize = PriorityReportPageSize();
        var page = FindingPagination.Slice(_sortedPriorityFindings, _priorityReportPage, pageSize);
        _priorityReportPage = page.PageIndex;
        _visiblePriorityFindings.ReplaceRange(page.Items);
        PriorityReportDescriptionText.Text = $"{LocalizationService.Get("PriorityReportDescription")} · {_sortedPriorityFindings.Length:N0} / {_priorityAuditSession.FindingIds.Count:N0}";
        PriorityReportPageStatus.Text = string.Format(LocalizationService.Get("PageStatus"), page.PageIndex + 1, page.PageCount, page.TotalCount, page.PageSize);
        PriorityReportPreviousButton.IsEnabled = page.PageIndex > 0;
        PriorityReportNextButton.IsEnabled = page.PageIndex + 1 < page.PageCount;
    }

    bool IsPriorityReportImagesFilter =>
        string.Equals((PriorityReportCategoryFilter?.SelectedItem as ComboBoxItem)?.Tag?.ToString(), "Images", StringComparison.OrdinalIgnoreCase);

    int PriorityReportPageSize() => FindingPagination.PresentationPageSize(
        _priorityReportTileMode, IsPriorityReportImagesFilter, PriorityReportTileZoom.Value);

    void UpdatePriorityReportPresentation()
    {
        if (PriorityReportImageModeControls is null) return;
        var isImages = IsPriorityReportImagesFilter;
        if (!isImages) _priorityReportTileMode = false;
        PriorityReportImageModeControls.Visibility = isImages ? Visibility.Visible : Visibility.Collapsed;
        PriorityReportGrid.Visibility = _priorityReportTileMode ? Visibility.Collapsed : Visibility.Visible;
        PriorityReportTileList.Visibility = _priorityReportTileMode ? Visibility.Visible : Visibility.Collapsed;
        PriorityReportTileScaleControls.Visibility = _priorityReportTileMode ? Visibility.Visible : Visibility.Collapsed;
        PriorityReportViewToggle.Content = LocalizationService.Get(_priorityReportTileMode ? "ListView" : "TileView");
    }

    void TogglePriorityReportView_Click(object sender, RoutedEventArgs e)
    {
        if (!IsPriorityReportImagesFilter) return;
        _priorityReportTileMode = !_priorityReportTileMode;
        UpdatePriorityReportPresentation();
        RefreshPriorityReport(true);
    }

    void PriorityReportTileZoom_Changed(object sender, RoutedPropertyChangedEventArgs<double> e)
    {
        if (PriorityReportTileZoomLabel is null) return;
        PriorityReportTileZoomLabel.Text = $"{e.NewValue:0} px";
        if (_priorityReportTileMode) RefreshPriorityReport(true);
    }

    void PriorityReportFilter_Changed(object sender, SelectionChangedEventArgs e)
    {
        UpdatePriorityReportPresentation();
        RefreshPriorityReport(true);
    }
    void PriorityReportSearch_Changed(object sender, TextChangedEventArgs e) => RefreshPriorityReport(true);
    void PriorityReportReset_Click(object sender, RoutedEventArgs e)
    {
        ...
        _priorityReportTileMode = false;
        UpdatePriorityReportPresentation();
        RefreshPriorityReport(true);
    }

    void PriorityReportPreviousPage_Click(object sender, RoutedEventArgs e)
    {
        if (_priorityReportPage <= 0) return;
        _priorityReportPage--;
        RefreshPriorityReport();
    }

    void PriorityReportNextPage_Click(object sender, RoutedEventArgs e)
    {
        var pageCount = Math.Max(1, (int)Math.Ceiling(_sortedPriorityFindings.Length / (double)PriorityReportPageSize()));
        if (_priorityReportPage + 1 >= pageCount) return;
        _priorityReportPage++;
        RefreshPriorityReport();
    }
    ...
    void PriorityReportTile_Selected(object sender, SelectionChangedEventArgs e)
    {
        if (PriorityReportTileList.SelectedItem is Finding finding) SelectFinding(finding);
    }

    void PriorityReportTile_DoubleClick(object sender, MouseButtonEventArgs e)
    {
        var element = e.OriginalSource as DependencyObject;
        while (element is not null && element is not ListBoxItem) element = VisualTreeHelper.GetParent(element);
        if (element is ListBoxItem { DataContext: Finding finding })
        {
            SelectFindingAndShowDetails(finding, DetailsReturnSource.PriorityReportTiles);
            e.Handled = true;
        }
    }
    ...
    async void Scan_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            ApplyPersonalState();
            _mediaFindings.AddRange(result.Findings.Where(x => x.Category is "Images" or "Video").OrderByDescending(PrivacyRadarRanking.Score).ThenByDescending(x => x.ModifiedAt ?? DateTime.MinValue));
            UpdatePeoplePresentation();
    ...
    void ResetPriorityAuditForNewAudit()
    {
        ...
        if (_priorityPreparationCts is not null) RequestCancellation(_priorityPreparationCts, showGlobalProgress: false);
        ...
        _sortedPriorityFindings = [];
        _priorityReportPage = 0;
        _findingsPriorityPartialHintDismissed = false;
        _mediaPriorityPartialHintDismissed = false;
        _applicationHistoryPriorityPartialHintDismissed = false;
        TabPriorityReport.Visibility = Visibility.Collapsed;
        PriorityAuditWizardPanel.Visibility = Visibility.Collapsed;
        PrivacyRadarIntroPanel.Visibility = Visibility.Visible;
    }
    ...
    void UpdatePriorityPartialHints()
    {
        var completed = _priorityAuditSession?.Status == PriorityAuditStatus.Completed;
        if (FindingsPriorityPartialHintPanel is not null)
            FindingsPriorityPartialHintPanel.Visibility = completed && !_findingsPriorityPartialHintDismissed ? Visibility.Visible : Visibility.Collapsed;
        if (MediaPriorityPartialHintPanel is not null)
            MediaPriorityPartialHintPanel.Visibility = completed && !_mediaPriorityPartialHintDismissed ? Visibility.Visible : Visibility.Collapsed;
        if (ApplicationHistoryPriorityPartialHintPanel is not null)
            ApplicationHistoryPriorityPartialHintPanel.Visibility = completed && !_applicationHistoryPriorityPartialHintDismissed ? Visibility.Visible : Visibility.Collapsed;
    }

    void FindingsPriorityPartialHintClose_Click(object sender, RoutedEventArgs e) { _findingsPriorityPartialHintDismissed = true; UpdatePriorityPartialHints(); }
    void MediaPriorityPartialHintClose_Click(object sender, RoutedEventArgs e) { _mediaPriorityPartialHintDismissed = true; UpdatePriorityPartialHints(); }
    void ApplicationHistoryPriorityPartialHintClose_Click(object sender, RoutedEventArgs e) { _applicationHistoryPriorityPartialHintDismissed = true; UpdatePriorityPartialHints(); }
            ...
            var prepared = await Task.Run(() =>
            {
                ...
                foreach (var finding in mediaSnapshot)
                {
                    ...
                    var matches = filter switch
                    {
                        ...
                        "Documents" => isDoc,
                        "NSFW" => ImageSafetyMetadata.IsHighConfidenceNsfw(finding.MetadataJson),
    ...
    void ApplyPeopleResults(IEnumerable<PeopleScanResult> results)
    {
        var byPath = _findings.GroupBy(x => x.Path, StringComparer.OrdinalIgnoreCase)
    ...
    async Task RefreshImageSafetyModelAvailabilityAsync()
    {
        try { _imageSafetyModelInstalled = await _imageSafetyModelManager.IsInstalledAsync(); }
        catch { _imageSafetyModelInstalled = false; }
        _imageSafetyModelStatusKnown = true;
        UpdateModelControls();
    }
    ...
    async void DownloadNsfwModel_Click(object sender, RoutedEventArgs e)
    {
        if (_cts is not null) return;
        ...
        try
        {
            ...
            {
                NsfwScanStageText.Text = p.Stage.ToString();
                NsfwScanProgress.IsIndeterminate = p.Fraction is null;
        ...
        catch (OperationCanceledException) { NsfwScanStageText.Text = LocalizationService.Get("NsfwScanCanceled"); }
        catch (Exception ex)
        {
            NsfwScanStageText.Text = LocalizationService.Get("NsfwModelRequired"); NsfwScanErrorText.Text = ex.Message; NsfwScanErrorText.Visibility = Visibility.Visible;
        }
        finally { guard.Dispose(); _cts.Dispose(); _cts = null; UpdateModelControls(); }
    }
    ...
    async void NsfwScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!await _imageSafetyModelManager.IsInstalledAsync())
        {
            if (System.Windows.MessageBox.Show(LocalizationService.Get("NsfwModelPrompt"), LocalizationService.Get("AppTitle"), MessageBoxButton.YesNo, MessageBoxImage.Information, MessageBoxResult.No) == MessageBoxResult.Yes)
                DownloadNsfwModel_Click(sender, e);
            return;
        }
        ...
        var completedThisRun = new ConcurrentQueue<ImageSafetyScanResult>();
        try
        {
            ...
            var results = await new ImageSafetyScanner(_imageSafetyModelManager, _imageSafetyRepository).ScanAsync(images, false, progress, _imageSafetyScanCts.Token, completedThisRun.Enqueue);
            ApplyImageSafetyResults(results);
        ...
        {
            ApplyImageSafetyResults(completedThisRun);
            SaveCurrentSnapshot(); UpdatePeoplePresentation(); RefreshFindingsPage(true);
    ...
    void ApplyImageSafetyResults(IEnumerable<ImageSafetyScanResult> results)
    {
        var byPath = _findings.GroupBy(x => x.Path, StringComparer.OrdinalIgnoreCase)
            .ToDictionary(group => group.Key, group => group.First(), StringComparer.OrdinalIgnoreCase);
        foreach (var result in results)
        {
            if (result.Status != ImageSafetyScanStatus.Completed || !byPath.TryGetValue(result.Path, out var finding)) continue;
            finding.MetadataJson = ImageSafetyMetadata.InjectIntoMetadata(finding.MetadataJson, result);
            finding.MetadataJson = DetectionEvidenceCalculator.MarkCompleted(finding.MetadataJson, DetectionEvidenceCalculator.ImageSafety);
        }
    }
    ...
    {
        if (!_imageSafetyModelManager.HasModelFiles || _imageSafetyScanCts is not null) return;
        if (System.Windows.MessageBox.Show(LocalizationService.Get("NsfwModelRemovePrompt"), LocalizationService.Get("AppTitle"), MessageBoxButton.YesNo, MessageBoxImage.Warning, MessageBoxResult.No) != MessageBoxResult.Yes) return;
        try { _imageSafetyModelManager.RemoveInstalledModel(); _imageSafetyModelInstalled = false; _imageSafetyModelStatusKnown = true; NsfwScanStageText.Text = LocalizationService.Get("NsfwModelRemoved"); StatusText.Text = NsfwScanStageText.Text; UpdateModelControls(); }
    ...
    async void PeopleScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        var completedThisRun = new ConcurrentQueue<PeopleScanResult>();
        try
        {
            ...
            var results = await new PeopleScanner(_modelManager, _peopleRepository).ScanAsync(images, progress, _peopleScanCts.Token, completedThisRun.Enqueue);
            ApplyPeopleResults(results);
        ...
        {
            ApplyPeopleResults(completedThisRun);
            SaveCurrentSnapshot(); UpdatePeoplePresentation(); RefreshFindingsPage(true);
    ...
    async void PiiScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        var candidates = _findings.Where(x => !x.Ignored && TextExtractor.IsSupported(x.Path)).ToArray();
    ...
    async void ConfigsScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        var candidates = _findings.Where(x => !x.Ignored).ToArray();
        if (candidates.Length == 0)
        {
            guard.Dispose();
            StatusText.Text = LocalizationService.Get("NoTextFilesForScan");
    ...
    async void ArchivesScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        var candidates = _findings.Where(x => !x.Ignored && ArchiveInspector.IsSupportedArchive(x.Path)).ToArray();
        if (candidates.Length == 0)
        {
            guard.Dispose();
            StatusText.Text = LocalizationService.Get("NoTextFilesForScan");
    ...
    async void ExifScan_Click(object sender, RoutedEventArgs e)
    {
        ...
        await Task.Yield();
        Finding[] candidates;
        try { candidates = await Task.Run(() => _findings.Where(x => !x.Ignored).ToArray(), token); }
    ...
    void MediaDeepAnalysisToggle_Click(object sender, RoutedEventArgs e)
    {
        var collapse = MediaDeepAnalysisContent.Visibility == Visibility.Visible;
        MediaDeepAnalysisContent.Visibility = collapse ? Visibility.Collapsed : Visibility.Visible;
        MediaDeepAnalysisToggle.ToolTip = LocalizationService.Get(collapse ? "ExpandDeepAnalysisHelp" : "CollapseDeepAnalysisHelp");
    }
    ...
    void SelectFindingAndShowDetails(Finding f, DetailsReturnSource source)
    {
        var sourceControl = source switch
        {
            ...
            DetailsReturnSource.PriorityReportTiles => PriorityReportTileList,
        ...
        _ = Dispatcher.BeginInvoke(System.Windows.Threading.DispatcherPriority.ContextIdle, new Action(() =>
        {
            ...
            switch (state.Source)
            {
                ...
                case DetailsReturnSource.PriorityReport:
                    ...
                    break;
                case DetailsReturnSource.PriorityReportTiles:
                    PriorityReportTileList.SelectedItem = state.Finding;
                    break;
    ...
    void SelectFinding(Finding f, bool keepSimilarSection = false)
    {
        ...
        DeleteDetailsFileButton.Visibility = IsDirectoryFinding(f) ? Visibility.Collapsed : Visibility.Visible;
        ...
        var peopleDetails = PeopleScanMetadata.TryParse(f.MetadataJson, out var people)
            ...
            : "";
        var safetyDetails = ImageSafetyMetadata.TryParse(f.MetadataJson, out var safety)
            ? $"\n\n{string.Format(LocalizationService.Get("NsfwScoreDetails"), safety!.PrimaryClass, safety.NsfwScore, safety.NsflScore, safety.SfwScore, safety.ModelVersion, safety.ScannedAtUtc.ToLocalTime())}"
            : "";
        ...
        DetailsText.Text = $"{f.DisplayName}\n\nFull path:\n{f.Path}\n\nSize: {f.SizeDisplay}\nCreated: {f.CreatedAt}\nModified: {f.ModifiedAt}\nLast access: {f.LastAccessAt}\n\nExposure: {f.ExposureScore} / 100 ({f.RiskLevel})\n{radarDetails}\nReasons:\n• {string.Join("\n• ", f.ExposureReasons)}\n\nCategory: {f.Category}\nSubcategory: {f.Subcategory}\nAge: {f.AgeClass}\nScanner: {f.ScannerId}{personalDetails}{piiDetails}{secDetails}{docDetails}{peopleDetails}{safetyDetails}{exifDetails}{configDetails}{identityDetails}{archiveDetails}{applicationHistoryDetails}";
    ...
    void DeleteDetailsFile_Click(object sender, RoutedEventArgs e)
    {
        if (_selected is not null && !IsDirectoryFinding(_selected)) DeleteFinding(_selected);
    }
    ...
    void DeleteFindings(IReadOnlyCollection<Finding> selected)
    {
        ...
        try
        {
            ...
            if (_selected is not null && removedKeys.Contains(PersonalAttentionFeatureExtractor.PathKey(_selected.Path)))
            {
                _selected = null;
                DetailsText.Clear();
                OpenDetailsFileButton.Visibility = Visibility.Collapsed;
                DeleteDetailsFileButton.Visibility = Visibility.Collapsed;
                DetailsImagePreviewBorder.Visibility = Visibility.Collapsed;
            }
            ...
            PriorityReportTileList.UnselectAll();
            SaveCurrentSnapshot();
            RefreshFindingsPage(true);
            RefreshPriorityReport();
            BuildDashboard();
    ...
    public bool CanRunCleanup => _activeHeavyTaskName is null && _activeMediaTaskCount == 0 && _cts is null && _personalTrainingCts is null && _priorityPreparationCts is null &&
    ...
    void CancelAllBackgroundWork()
    {
        ...
        _mediaFilterCts?.Cancel();
        _restoreCts?.Cancel();
        _priorityPreparationCts?.Cancel();
